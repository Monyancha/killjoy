...
mysqli_select_db( $stomer, $database_stomer);
$query_image_path = sprintf("SELECT * FROM tbl_prodimages WHERE sessionid = %s ORDER BY upload_time ASC", GetSQLValueString($colname_image_path, "text"));
...
mysqli_select_db( $stomer, $database_stomer);
$query_featured_image = sprintf("SELECT * FROM tbl_prodimages WHERE sessionid = %s AND featured_image = 1", GetSQLValueString($colname_featured_image, "text"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "events")) {
	...
	if ($_POST['classification'] == 1) {
		...
		$newherb = strtoupper($_POST['product']);
	
  $insertSQL = sprintf("INSERT INTO tbl_herbs (herb_name) VALUES (%s)",
                       GetSQLValueString($newherb , "text"));

  mysqli_select_db( $stomer, $database_stomer);
  $Result1 = mysqli_query( $stomer, $insertSQL) or die(mysqli_error($GLOBALS["___mysqli_ston"]));
  
	} else {
		if ($_POST['classification'] == 2) {
			
			$newveg = strtoupper($_POST['product']);
		
		  $insertSQL = sprintf("INSERT INTO tbl_vegetables (vegtable_name) VALUES (%s)",
                       GetSQLValueString($newveg, "text"));
...
<h1>St. Omer Farm - Create new Products</h1></div>
...
<table border="0" cellpadding="0" cellspacing="0" class="contacttbl">
  <tr>
    <td><h2 class="inputs">Add a new Herb, Vegetable or Plant </h2></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
  ...
  <tr>
    <td class="labels">Produt Name:</td>
  </tr>
  <tr>
    <td class="inputs"><label>
      <input name="product" type="text" class="inputfields" id="product" />
      </label></td>
  </tr>
  <tr>
...
url     : "../functions/prod_session.php",
...
url     : "../functions/setfeaturedproductimage.php",
...
async   : false,
data    : { "image_id" : image_id }, 
url     : "../functions/unlinkproductpreviewimage.php",
