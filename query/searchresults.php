...
$query_rs_search_results = "SELECT DISTINCT (SELECT COUNT(tbl_address_comments.sessionid) FROM tbl_address_comments WHERE tbl_address_comments.sessionid = tbl_address.sessionid) AS reviewCount, city as city, str_number as strNumber, IFNULL(tbl_propertyimages.image_url,'https://www.killjoy.co.za/images/icons/house-outline-bg.png') AS propertyImage, IFNULL(tbl_approved.is_approved,'0') as Status, street_name AS Street, tbl_address.sessionid As id, city as Town FROM tbl_address LEFT JOIN tbl_address_comments ON tbl_address_comments.sessionid = tbl_address.sessionid LEFT JOIN tbl_propertyimages ON tbl_propertyimages.sessionid = tbl_address.sessionid LEFT JOIN tbl_approved ON tbl_approved.address_comment_id = tbl_address_comments.id WHERE tbl_address.street_name LIKE '%$my_data%' OR tbl_address.str_number LIKE '%$my_data%' OR city like '%$my_data%' GROUP BY tbl_address.sessionid ORDER BY Street ASC";
...
<div class="formheader">Showing results for <span class="mydata"><?php echo $my_data ?></span></div>
  ...
  <?php echo $row_rs_search_results['reviewCount'] ?>
    ...
    </div><?php } // Show if recordset not empty ?></div>
